...
Run `./compose/check-previews.sh` after startup to verify every page, cross-box material route, the Engine catalog and its served worlds, and the Quests build. `node compose/check-launcher.mjs [small|medium|large]` creates a template city, opens free play, then saves and resumes. `node compose/check-game.mjs <size> --theme "..." --brief "..."` makes a whole story game: it names the city in the theme, opens interiors, has the model server write the story against them, checks the shipped bundle and prints each stage's seconds and the play URL; `node compose/check-story.mjs <cityId>` does the same for the recorded story on a city that exists. `node compose/check-voice.mjs` speaks one fresh line and checks its header, first audio under 2 s and real-time factor under 1.5; it skips when voice is not running. `node compose/play-probe.mjs <world id>` walks up to a person and chats in that world's read-only preview in a headless Brave or Chrome, and writes screenshots and a report under the OS temp dir; `follow` and `lead` ask someone along from the chat's actions and walk with them, and `scene` visits each quest scene staged at quest start.
