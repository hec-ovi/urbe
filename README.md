...
A deterministic city sandbox. The same inputs reproduce the street plan, buildings, rooms and aggregate population byte-identically. The same ordered interaction history reproduces every instanced NPC. Agents write names, NPC types and stories through the naming and quest pipelines, a local language model answers NPC dialogue in game, and a three.js WebGPU client plays the result first person at street level.
...
5. **Name and tell.** An author agent names every district, station, line and business against a theme and writes the themed NPC types, through the naming pipeline's stages and checks. The quest pipeline has the author write narrative first, adapts it into validated branching gameplay, and bundles objectives, investigation scenes, mission objects, fixed interaction anchors and host capability checks for the engine.
...
Geometry and aggregate population are pure functions of their declared inputs. Instanced NPCs also depend on ordered interaction history. Authoring stays outside the deterministic geometry and population code paths, and city creation asks no model.
...
Run `./compose/check-previews.sh` after startup to verify every page, cross-box material route, the Engine catalog and its served worlds, and the Quests build. `node compose/check-launcher.mjs [small|medium|large]` creates a template city, opens free play, then saves and resumes. `node compose/check-game.mjs <size> --recording <dir> [--plan <id> | --seed <seed>] [--named <dir>]` makes a whole story game from pre-authored work: it plans the city (or takes a plan Engine holds), builds it from the plan as an author named it, opens interiors, imports the authored story against them, checks the shipped bundle and prints each stage's seconds and the play URL; `node compose/check-story.mjs <cityId>` plays the recorded story on an unnamed city that exists. `node compose/check-voice.mjs` speaks one fresh line and checks its header, first audio under 2 s and real-time factor under 1.5; it skips when voice is not running. `node compose/play-probe.mjs <world id>` walks up to a person and chats in that world's read-only preview in a headless Brave or Chrome, and writes screenshots and a report under the OS temp dir; `follow` and `lead` ask someone along from the chat's actions and walk with them, `scene` visits each quest scene staged, after `--advance-to <step>` fast-forwards the story when given, `story` plays the main questline step by step to an ending, and `ui` checks Escape in a conversation and the pause menu.
...
The [Engine launcher](http://localhost:5306/) creates cities from Small (500 m), Medium (1000 m) or Big (3000 m) templates. Next builds streets and exteriors; Play without quests opens a saved free-play game. Interiors and story are optional. Each stage runs as an Engine creation job, so a stage that builds for minutes holds no request open. For a named story game the launcher API plans a city, builds it from the plan as an author named it and imports a story an author recorded against its opened interiors; Engine checks what they wrote and asks no model. Catalog games can also carry a named blueprint and validated quest bundle. The other pages isolate one layer so geometry, data and materials can be inspected before assembly. Port 5306 defaults to WebGPU; add `&backend=webgl` to an Engine URL for its WebGL fallback.

Quests runs inside Compose without a public port because it watches and rebuilds the library consumed by Engine. Streets typechecks inside Compose without a public port; Engine builds street pieces during city assembly. Naming is a CLI and library; the `naming` service runs its CLI for an author under the `author` profile and never starts with the stack.

### Authoring a named story game

An author agent writes the names and the story through the Naming and Quests pipelines; Engine plans, builds, replays and checks, and asks no model. With the stack up, from this checkout (`<id>` is the plan's id, `<size>` its size, `<name>` the story's folder):

1. Plan: `planCity {size}` as an Engine creation job (`POST /api/creation-jobs`) writes `engine/out/plans/<id>/blueprint.json`.
2. Name: `docker compose run --rm -T -v "$PWD/engine/out/plans/<id>:/plan" naming sh -c 'npm run --silent build >&2 && npm run --silent world -- /plan --theme "<theme>" --external /plan/author'`. Exit 2 lists the requests: write each answer as the [Naming author guide](naming/AUTHORING.md) says and run it again, until exit 0 writes `blueprint.named.json` and `npc-types.json` beside the plan.
3. Build: `buildCity {cityId: "<id>", named: {blueprint: "out/plans/<id>/blueprint.named.json", types: "out/plans/<id>/npc-types.json"}}`, then `generateInstances {cityId: "<id>", mode: "automatic", count: 9, buildingIds: []}`. The plan's naming work moves into the city's `naming/`; the draft `engine/out/drafts/<id>` holds the world, its people and the game's capabilities the story is written against.
4. Tell: `mkdir -p engine/out/stories`, then `docker compose run --rm --no-deps -T -v "$PWD/engine/out/drafts/<id>:/draft:ro" -v "$PWD/engine/out/stories:/stories" quests npm run --silent author -- --world /draft/blueprint.json --types /draft/npc-types.json --handoff /draft/quests/handoff-input.json --parcels=<opened ids> --profile <size> --mechanics goto,observe,talk,listen,pickup,deliver,steal,work,investigation,escort --external /stories/<name>-author --out /stories/<name>`, answering as the [Quests author guide](quests/AUTHORING.md) says until exit 0.
5. Play: `importStory {cityId: "<id>", recording: "out/stories/<name>", sideJobs: 3}`, then `createGame`, or `node compose/check-game.mjs <size> --plan <id> --named out/plans/<id> --recording out/stories/<name>` for steps 3 to 5 with checks.

Cross-box data is mounted read-only where a preview needs it: connections reads the Atlas sample blueprint; Exterior and Interior read the Materials theme database; Engine reads the Atlas samples and built CLI, Connections, Exterior and Interior source, the Simulation and Quests builds, the Materials theme database, and the machine's model store (`URBE_MODELS_DIR`, default `~/models/quaternius`). Compose also mounts the sibling dependency volumes required by those imports and CLIs. The Materials sphere viewer only reads the committed database, so it needs no ComfyUI. Engine also reads Quests with its installed dependencies and runs its replay CLI to play a recorded story against a city's opened interiors. Quests is a library whose build Engine imports; questline runtime runs in the browser, while NPC dialog uses the machine's OpenAI-compatible model server at `LLM_BASE_URL`, default host port 8080. Names and stories are not written at runtime.
...
| [urbe-namer](https://github.com/hec-ovi/urbe-namer) | naming | Naming pass and themed NPC type set |
...
Data flows `atlas -> connections/base -> exterior kit -> engine placement tables`, `atlas -> streets -> engine`, `buildingforge -> connections/rooftop-spans -> engine`, `atlas plan -> naming -> engine assembly` for a named city, `atlas -> naming -> simulation -> quests -> engine`, and `simulation -> engine -> voice -> engine` for NPC speech; pbrforge feeds the two geometry tools and the engine. Interior modules and layouts enter Engine beside the placement tables.
