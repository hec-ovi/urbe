...
Run `./compose/check-previews.sh` after startup to verify every page, cross-box material route, the Engine catalog and its served worlds, and the Quests build. `node compose/check-launcher.mjs [small|medium|large]` creates a template city, opens free play, then saves and resumes. `node compose/check-game.mjs <size> --recording <dir> [--named <dir> --seed <seed>]` makes a whole story game from pre-authored work: it plans the city, builds it from the plan as an author named it, opens interiors, imports the recorded story against them, checks the shipped bundle and prints each stage's seconds and the play URL; `node compose/check-story.mjs <cityId>` plays the recorded story on an unnamed city that exists. `node compose/check-voice.mjs` speaks one fresh line and checks its header, first audio under 2 s and real-time factor under 1.5; it skips when voice is not running. `node compose/play-probe.mjs <world id>` walks up to a person and chats in that world's read-only preview in a headless Brave or Chrome, and writes screenshots and a report under the OS temp dir; `follow` and `lead` ask someone along from the chat's actions and walk with them, and `scene` visits each quest scene staged at quest start.
...
Quests runs inside Compose without a public port because it watches and rebuilds the library consumed by Engine. Streets typechecks inside Compose without a public port; Engine builds street pieces during city assembly. Naming is a CLI and library that authors run outside the stack.
